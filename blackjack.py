...
def load_images(card_images):
    ...
    for suit in suits:
        ...
        for card in range(1, 11):
            ...
            if card == 1:
                card_images.append(('A' + suit[0].upper(), image, ))
            else:
                card_images.append((str(card) + suit[0].upper(), image, ))

        # next the face cards
        for card in face_cards:
            name = 'cards/{}_{}.{}'.format(str(card), suit, extension)
            image = tkinter.PhotoImage(file=name)
            card_images.append((card[0].upper()+suit[0].upper(), image, ))
...
def deal_player():
    ...
    name = BJ.hit('player')
    tkinter.Label(player_card_frame, image=deck.get(name), relief="raised").pack(side="left")
    player_score = score_hand('player')
...
def initial_deal():
    ...
    #dealer_hand.append(_deal_card(dealer_card_frame))
    BJ.initDealer()
    dealer_score_label.set(score_hand('dealer'))
    deal_player()
